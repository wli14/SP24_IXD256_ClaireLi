...
#### Used (light_sensor.read()) to read a value from light sensor, assuming it returns a value in the range of 0 to 4095 (a 12-bit ADC value), then maps this value to a range between 60 and 170 using the map_value function. Compare the current servo_val with the servo_val_final. If the current servo_val is less than servo_val_final, it increments servo_val by 1. If it's greater, it decrements servo_val by 1. Finally, the servo loop to the new position represented by servo_val.
